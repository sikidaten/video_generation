...
if __name__ == '__main__':
    model = VQVAE(optimizer=torch.optim.Adam, activation=nn.ReLU(),z_feature=1,feature=256)
    print(model)
    img = torch.randn(8, 3, 128, 128)
    loss = model.batch(img, 'train')
    testinput=torch.randint(0, 512, [8 , 32 , 32])
    model.generate(testinput)
